...
public class AddTripController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int bookedTicketNumber = request.getParameter("bookedTicketNumber")==null?0:Integer.parseInt(request.getParameter("bookedTicketNumber"));
		String carType = request.getParameter("carType");
		Date departureDate = DateUtils.convertStringToDate(request.getParameter("departureDate"));
		Time departureTime = Time.valueOf(request.getParameter("departureTime") + (request.getParameter("departureTime").length() < 8 ? ":00" : ""));
