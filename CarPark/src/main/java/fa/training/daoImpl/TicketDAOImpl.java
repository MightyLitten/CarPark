...
public class TicketDAOImpl implements TicketDAO {
	...
	public Boolean deleteTicketByCar(String plate) throws SQLException {
		...
		try {
			connection = DBUtils.getInstance().getConnection();
			preparedStatement = connection.prepareStatement(SQLCommand.QUERY_DELETE_TICKET_BY_CAR);
	...
	public Boolean deleteTicketByParking(long id) throws SQLException {
		...
		try {
			connection = DBUtils.getInstance().getConnection();
			preparedStatement = connection.prepareStatement(SQLCommand.QUERY_DELETE_TICKET_BY_PARKING);
			preparedStatement.setLong(1, id);
			check = preparedStatement.executeUpdate() == 1;
		} finally {
			try {
