				...
				<div class="card-body">
					<form action="<%=request.getContextPath()%>/AddParkingServlet"
						...
						<c:if test="${edit}">
							<div class="row">
								<div class="col-md-3">
									<label for="trip"><b>Status</b></label>
								</div>
								<div class="form-group col-md-7">
									<select class="form-control" name="status" id="status">
										<option value="Active"
											${parkingLot.status eq "Active" ? 'selected="selected"' : ''}>Active</option>
										<option value="Inactive"
											${parkingLot.status eq "Inactive" ? 'selected="selected"' : ''}>Inactive</option>
									</select>
								</div>
							</div>
						</c:if>
